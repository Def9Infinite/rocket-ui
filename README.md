...
- **Launch site & weather** — pick a pad (14 of them, from Cape Canaveral to Satish Dhawan SLP in India); live weather is pulled from Open‑Meteo (falls back to simulated if offline) and a launch GO/NO‑GO is computed. Pad names are localized too — in 中文 the picker and overlays read 文昌 LC‑101 — 中国 海南.
...
- **Vehicle presets** — quick‑load a built‑in vehicle spec (Falcon 9 Block 5, Long March 2F, Long March 5, Electron, GSLV Mk II) with `LOAD PRESET`. Every preset ships **both an English and a Chinese spec** — load it while the panel is in 中文 and the whole stage breakdown comes in Chinese (长征二号F, 4 × YF‑20B（芯级）, 四氧化二氮 / 偏二甲肼 …). Switching language doesn't retranslate an already‑loaded spec — re‑click `LOAD PRESET`.
